...
namespace TaskTupleAwaiter.Tests.Adapters
{
	...
	public abstract partial class AwaiterAdapter
	{
		...
		private AwaiterAdapter(string description) => _description = description;
			...
			bool? continueOnCapturedContext = null)
		{
			if (continueOnCapturedContext != null)
			{
				...
				return tasks.Length switch
				{
					...
					10 => new ConfiguredTaskTupleAwaiter10Adapter(
						(tasks[0], tasks[1], tasks[2], tasks[3], tasks[4],
							tasks[5], tasks[6], tasks[7], tasks[8], tasks[9])
						.ConfigureAwait(continueOnCapturedContext.Value)
						.GetAwaiter(), description),
					_ => throw new ArgumentException(
						$"There is no awaiter for a tuple of {tasks.Length} tasks.",
						nameof(tasks))
				};
			}
			else
			{
				...
				return tasks.Length switch
				{
					1 => new TaskTupleAwaiter1Adapter(
						ValueTuple.Create(tasks[0]).GetAwaiter(),
						"await ValueTuple.Create(t)"),
					2 => new TaskTupleAwaiter2Adapter(
						(tasks[0], tasks[1]).GetAwaiter(), description),
					3 => new TaskTupleAwaiter3Adapter(
						(tasks[0], tasks[1], tasks[2]).GetAwaiter(),
						description),
					4 => new TaskTupleAwaiter4Adapter(
						(tasks[0], tasks[1], tasks[2], tasks[3]).GetAwaiter(),
						description),
					5 => new TaskTupleAwaiter5Adapter(
						(tasks[0], tasks[1], tasks[2], tasks[3], tasks[4])
						.GetAwaiter(), description),
					6 => new TaskTupleAwaiter6Adapter(
						(tasks[0], tasks[1], tasks[2], tasks[3], tasks[4],
							tasks[5]).GetAwaiter(), description),
					7 => new TaskTupleAwaiter7Adapter(
						(tasks[0], tasks[1], tasks[2], tasks[3], tasks[4],
							tasks[5], tasks[6]).GetAwaiter(), description),
					8 => new TaskTupleAwaiter8Adapter(
						(tasks[0], tasks[1], tasks[2], tasks[3], tasks[4],
							tasks[5], tasks[6], tasks[7]).GetAwaiter(),
						description),
			...
			bool? continueOnCapturedContext = null)
		{
			...
			else
			{
				...
				return tasks.Length switch
				{
					1 => new VoidResultTaskAwaiterAdapter(
						ValueTuple.Create(tasks[0]).GetAwaiter(),
						"await ValueTuple.Create(t)"),
					2 => new VoidResultTaskAwaiterAdapter(
						(tasks[0], tasks[1]).GetAwaiter(), description),
					3 => new VoidResultTaskAwaiterAdapter(
						(tasks[0], tasks[1], tasks[2]).GetAwaiter(),
						description),
					4 => new VoidResultTaskAwaiterAdapter(
						(tasks[0], tasks[1], tasks[2], tasks[3]).GetAwaiter(),
						description),
					5 => new VoidResultTaskAwaiterAdapter(
						(tasks[0], tasks[1], tasks[2], tasks[3], tasks[4])
						.GetAwaiter(), description),
